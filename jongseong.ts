...
function _codeForHangul(h: string): number{
...
//    16 ~ 19   |  경
//    20 ~ 23   |  해
function _codeForZeros(zs: string) : number{
  const n = zs.length
...
function _codeForDigit(d: string): number{
  return [21, 8, 0, 16, 0, 0, 1, 8, 8, 0][Number(d)]
}
...
function _codeForEnglish(e: string): number{
  return /ck/i.test(e) ? 1
  : /.n/i.test(e) ? 4
  : /ne/i.test(e) ? 4
  : /.l/i.test(e) ? 8
  : /le/i.test(e) ? 8
...
function _codeForEnglishInitial(e: string): number{
...
export function code(word: string): number{
  if (!word) {
    return 0
  }
...
export function hasJongseong(w: string): boolean{
  return code(w) !== 0
}
